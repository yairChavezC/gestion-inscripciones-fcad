...
const Cursos = () => {
  ...
  // Estados para Paginación y Búsqueda (Requisito de la cátedra)
  const [busqueda, setBusqueda] = useState('');
  const [paginaActual, setPaginaActual] = useState(0); 
  const [totalRegistros, setTotalRegistros] = useState(0);
  const limite = 10; 

  // --- CARGA DE DATOS (BROWSE) ---
  const cargarCursos = async () => {
    try {
      const offset = paginaActual * limite;
      // Enviamos paginación y criterio de búsqueda
  ...
  // Se ejecuta al cargar el componente o al cambiar de página
  useEffect(() => {
    cargarCursos();
  }, [paginaActual]); 
  ...
  // READ (Detalles)
  const handleDetalles = (curso) => {
    setModal({ tipo: 'detalles', datos: curso });
  };
  ...
  // EXTRAS: Diploma
  const handleDiploma = async (id_curso) => {
    alert(`Generando diploma para el curso ${id_curso}...`);
  ...
  return (
    <div className="page-content">
      ...
      {/* --- CABECERA: TÍTULO, BÚSQUEDA Y BOTÓN NUEVO --- */}
          ...
          <div className="course-card" key={curso.id_curso}>
            <div className="card-header">
              <div className="card-title-group">
                ...
                <div>
                  <h3 style={{ fontSize: '16px', margin: 0 }}>{curso.nombre}</h3>
                </div>
              </div>
            ...
            {/* BARRA DE ACCIONES CON ÍCONOS */}
            <div className="card-actions">
              <button 
                className="btn-action btn-detalles" 
                onClick={() => handleDetalles(curso)} 
                title="Ver Detalles"
              >
                <FiEye />
              </button>
              ...
              <button 
                className="btn-action btn-diploma" 
                onClick={() => handleDiploma(curso.id_curso)} 
      ...
      <div style={{ display: 'flex', justifyContent: 'center', gap: '15px', marginTop: '30px' }}>
        <button 
          ...
          style={{ padding: '8px 15px', cursor: paginaActual === 0 ? 'not-allowed' : 'pointer', border: '1px solid #ccc', borderRadius: '4px', backgroundColor: paginaActual === 0 ? '#f5f5f5' : 'white' }}
        >
          Anterior
        </button>
        ...
        <button 
          disabled={(paginaActual + 1) * limite >= totalRegistros} 
          onClick={() => setPaginaActual(paginaActual + 1)}
          style={{ padding: '8px 15px', cursor: (paginaActual + 1) * limite >= totalRegistros ? 'not-allowed' : 'pointer', border: '1px solid #ccc', borderRadius: '4px', backgroundColor: (paginaActual + 1) * limite >= totalRegistros ? '#f5f5f5' : 'white' }}
        >
          Siguiente
        </button>
      </div>
...
{/* --- MODAL MEJORADO --- */}
      {modal.tipo && (
        <div className="modal-overlay" onClick={cerrarModal}>
          {/* Evitamos que el clic en el modal cierre la ventana (propagación) */}
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            
            <div className="modal-header">
              <h3>
                {modal.tipo === 'crear' ? 'Crear Nuevo Curso' : 
                 modal.tipo === 'editar' ? 'Editar Curso' : 
                 'Detalles del Curso'}
              </h3>
              <button 
                onClick={cerrarModal} 
            ...
            {modal.tipo === 'detalles' ? (
              ...
              <form onSubmit={handleGuardar}>
                <div className="form-group">
                  ...
                  <input required type="text" name="nombre" value={modal.datos.nombre} onChange={handleChange} />
                </div>
                
                <div className="form-group">
                  <label>Descripción</label>
                  <textarea name="descripcion" value={modal.datos.descripcion} onChange={handleChange} />
                </div>

                <div className="form-group">
                  <label>Fecha de Inicio</label>
                  <input required type="date" name="fecha_inicio" value={modal.datos.fecha_inicio} onChange={handleChange} />
                </div>

                <div style={{ display: 'flex', gap: '15px' }}>
                  <div className="form-group" style={{ flex: 1 }}>
                    <label>Horas Totales</label>
                    <input required type="number" name="cantidad_horas" value={modal.datos.cantidad_horas} onChange={handleChange} />
                  </div>
                  <div className="form-group" style={{ flex: 1 }}>
                    <label>Cupo Máximo</label>
                    <input required type="number" name="inscriptos_max" value={modal.datos.inscriptos_max} onChange={handleChange} />
                  </div>
                </div>
